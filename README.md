<!-- Encabezado centrado con tu banner -->
<p align="center">
  <img src="Images/data_image.jpg">
</p>

<!-- Presentación -->
<h1 align="center">¡Hola! Soy <span style="color:#4da6ff;">Juan Daniel Hernández</span> 👋</h1>
<h3 align="center">Aspirante a Data Scientist | Apasionado por el análisis de datos, Python y Machine Learning 🚀</h3>

<br>

<!-- Sobre mí -->
<h2>🧩 Sobre mí</h2>

<p>
Soy biólogo en transición al mundo de la <strong>Ciencia de Datos</strong>, combinando mi enfoque analítico con habilidades técnicas para resolver problemas reales con datos.  
Actualmente estoy estudiando en el bootcamp <strong>TripleTen</strong> y desarrollando proyectos de EDA, modelos predictivos y análisis estadístico.
</p>

<ul>
  <li>📊 Estoy trabajando activamente en proyectos como <strong>ICE VideoGame Store Forecast</strong>, <strong>Megaline EDA + Predictive Model</strong>, y <strong>Zuber Chicago Weather Impact</strong></li>
  <li>🐍 Me especializo en Python, SQL, estadística aplicada, limpieza de datos y Machine Learning.</li>
  <li>📘 Tengo experiencia en MS Excel, análisis exploratorio y visualización.</li>
  <li>🎯 Mi objetivo: convertirme en Data Scientist Junior antes de finalizar el año.</li>
  <li>🤝 ¿Quieres conectar conmigo?  
    <a href="https://www.linkedin.com/in/juan-hernandez-vargas/">
      <img src="https://img.shields.io/badge/LinkedIn-Juan%20Hernandez-blue?style=flat&logo=linkedin">
    </a>
  </li>
</ul>

<br>

<!-- Lenguajes y herramientas -->
<h2>🛠️ Lenguajes y herramientas</h2>

<p>
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white">
  <img src="https://img.shields.io/badge/MySQL-00618A?style=for-the-badge&logo=mysql&logoColor=white">
  <img src="https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoft-excel&logoColor=white">
  <img src="https://img.shields.io/badge/Power%20BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white">
  <img src="https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white">
</p>

<br>

<!-- Proyectos destacados -->
<h2>📂 Proyectos destacados</h2>

<ul>
  <li>🎮 <strong>ICE VideoGame Store 2017 Forecast</strong> — EDA completo + predicciones con modelos supervisados.</li>
  <li>📱 <strong>Megaline Customer Behavior</strong> — análisis exploratorio + modelo de clasificación.</li>
  <li>🚕 <strong>Zuber Chicago</strong> — impacto climático en la demanda de viajes (SQL + Python + estadísticas).</li>
  <li>🏡 <strong>USA Housing Model</strong> — regresión, evaluación de modelos y métricas MAE/MSE/RMSE.</li>
</ul>

<br>

<!-- Estadísticas (con links a tu usuario) -->
<h2>🔥 Mis estadísticas</h2>

<p align="center">
  <!-- Stats generales -->
  <img src="https://github-readme-stats.vercel.app/api?username=Juanhv24&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" width="500"/>
  <br><br>

  <!-- Racha (streak) usando el nuevo dominio -->
  <img src="https://github-readme-streak-stats-eight.vercel.app?user=Juanhv24&theme=tokyonight&hide_border=true" width="500"/>
  <br><br>

  <!-- Lenguajes más usados -->
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Juanhv24&layout=compact&theme=tokyonight&hide_border=true" width="400"/>
</p>

<br>

<!-- Footer -->
<p align="center">
  <strong>Gracias por visitar mi perfil ✨</strong><br>
  Siempre abierto a colaborar y aprender cosas nuevas.
</p>



<!--
**Juanhv24/Juanhv24** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
